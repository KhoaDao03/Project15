...
def test_real_model_to_trade_history(store, raw, series, now, tmp_path, mode, side, preset, ending):
    ...
    def reference(when):
        ...
        send(
            ...
            dict(
                index_id="BRTI",
                data=json.dumps(dict(type="value", id="BRTI", time=when * 1000, value=str(value))),
            ),
            when,
        )
    ...
    try:
        ...
        send(
            ...
            dict(
                market_ticker=ticker,
                trade_id="causal-opposing-volume",
        ...
        assert e.executor.positions[ticker].quantity == 0.40
        ...
        with TestClient(
            create_app(store, settings=Settings(data_dir=str(tmp_path)), config=c, collect_live=False)
        ) as client:
        ...
        print(
            ...
            + json.dumps(
                dict(
                    ...
                    preset=preset,
                    ending=ending,
                    bought=float(sum(D(x["quantity"]) for x in buys)),
                    sold=float(sum(D(x["quantity"]) for x in sells)),
                    net_pnl=out["net_pnl"],
                    fees=float(fees),
                    complete=True,
                )
...
def test_lifecycle_deactivation_does_not_allow_reentry(scenario, store, market, now, mode, side):
    ...
    def ingest(kind, msg, when, ident):
        ...
        row = dict(
            id=ident,
            received=when,
            monotonic_ns=int((when - now + 100) * 1e9),
    ...
    ingest(
        "market_lifecycle_v2", dict(market_ticker=market.ticker, event_type="deactivated"), now, "deactivated"
    )
    assert not s.e.executor.orders
    b = make_book(side, ".88", ".90", now + 0.1)
    ingest(
        "orderbook_snapshot",
        dict(
...
@pytest.mark.parametrize("price", [0.891, 0.902, 0.912, 0.924, 0.929])
...
def test_passive_price_obeys_decimal_discount_on_supported_grid(market, now, side, ask):
    ...
    discount = min(
        D(c.passive_discount), (D(ask) - D(".91")) / 2, max(D(0), D(conservative) - D(ask) - D(c.min_edge))
    )
...
def test_sell_price_does_not_lose_an_extra_tick(store, market, now, mode, side):
    ...
    d = dict(decision="TRADE_CANDIDATE", side=side, conservative_probability=0.979)
    order = e.submit(market, b, d, "op", now, True)
    assert order is not None
    e.fill(order, 0.40, order.limit, now + 0.5, True)
    b = make_book(side, ".939", ".949", now + 1)
    p = {"conservative_" + side: 0.1}
